...
function generateOpenClawConfig(mountedConfig) {
  ...
  if (!Array.isArray(config.plugins.allow) || !config.plugins.allow.includes('openclaw-shadowob')) {
    config.plugins.allow = ['openclaw-shadowob']
  }
...
function startGateway(_healthServer) {
  ...
  const env = {
    ...
    OPENCLAW_DATA_DIR: OPENCLAW_DIR,
    OPENCLAW_STATE_DIR: join(OPENCLAW_DIR, 'state'),
...
async function main() {
  ...
  // 2c. Write workspace bootstrap files (SOUL.md, etc.) from ConfigMap
  // These files are mounted alongside config.json in /etc/xcloud/
  const WORKSPACE_BOOTSTRAP_FILES = ['SOUL.md', 'IDENTITY.md', 'TOOLS.md', 'AGENTS.md', 'USER.md', 'HEARTBEAT.md', 'BOOTSTRAP.md']
  const workspaceDir = openclawConfig.agents?.defaults?.workspace ?? SHARED_WORKSPACE_PATH
  if (workspaceDir) {
  ...
  // 2d. Ensure skills directory exists
  if (SKILLS_DIR) {
