...
describe('template-schema consistency (TPL-02)', () => {
  it.each(templateFiles)('%s is valid JSON', (file) => {
  ...
  it.each(templateFiles)('%s has required metadata fields', (file) => {
    const content = JSON.parse(readFileSync(resolve(templatesDir, file), 'utf-8'))
    // version is required
  ...
  it.each(templateFiles)('%s has deployments.agents array', (file) => {
    ...
    expect(content.deployments).toHaveProperty('agents')
    expect(Array.isArray(content.deployments.agents)).toBe(true)
  })

  it.each(templateFiles)('%s agents have id and runtime', (file) => {
    const content = JSON.parse(readFileSync(resolve(templatesDir, file), 'utf-8'))
    for (const agent of content.deployments.agents) {
  ...
  it.each(templateFiles)('%s agents have resource limits', (file) => {
    ...
    for (const agent of content.deployments.agents) {
      if (agent.resources) {
        ...
        expect(agent.resources.limits).toHaveProperty('memory')
      }
    }
  })
})
