 ...
 *
 * Pure mechanism. No bundled registry / presets — the template author is in
 * charge of telling the plugin where to clone from and which paths to mount
 * for which kind. Supports any combination of:
...
export function buildAgentPackPrompt(packs: ResolvedPack[], mountPath: string): string {
  ...
  for (const pack of packs) {
    const mountSummary = pack.mounts
      .map((mount) => `${mount.kind}: \`${mountPath}/${pack.id}/${mount.kind}\``)
      .join('; ')
  ...
  lines.push(
    ...
    '2. Use the mounted pack skills, commands, or sub-agents instead of improvising from memory.',
    '3. If the pack exposes `hooks/bootstrap.md` or `hooks/teardown.md`, follow those procedures when applicable.',
    '4. Run helper scripts from the mounted `scripts/` directory using absolute paths when they are useful.',
    '5. Cite the pack id and the exact skill, instruction file, or procedure you relied on.',
  )
...
export function resolvePacks(opts: AgentPackOptions): ResolvedPack[] {
  ...
  for (const p of packs) {
    ...
    const mounts: ResolvedMount[] = (p.mounts ?? []).map((m) => ({
      kind: m.kind,
      from: m.from,
      include: m.include,
    }))
...
const plugin = definePlugin(manifest as PluginManifest, (api) => {
  ...
  api.onValidate((context: PluginBuildContext): PluginValidationResult | void => {
    ...
    packs.forEach((p, i) => {
      ...
      if (!p.mounts || p.mounts.length === 0) {
        errors.push({
          path: `use.agent-pack.packs[${i}].mounts`,
          message:
            'Each pack must declare at least one mount, e.g. [{ "kind": "skills", "from": "skills" }].',
          severity: 'error',
        })
      } else {
