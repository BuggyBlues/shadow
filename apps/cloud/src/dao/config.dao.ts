...
import { type Config, type NewConfig, configs, configVersions } from '../db/schema.js'
...
export class ConfigDao {
  ...
  getVersionHistory(name: string): Array<{ version: number; content: unknown; message: string | null; createdAt: string | null }> {
    return this.db
      .select()
      .from(configVersions)
  ...
  getVersion(name: string, version: number): { version: number; content: unknown; createdAt: string | null } | undefined {
    return this.db
      .select()
      .from(configVersions)
      .where(eq(configVersions.configName, name))
