...
function StepConfigure({
  ...
  useEffect(() => {
    if (
      ...
      Object.keys(savedLookup).length === 0
    )
      return
    ...
    for (const key of requiredVars) {
      if (!merged[key] && savedLookup[key]) {
        // Auto-fill with __SAVED__ sentinel — backend resolves from stored secrets/env
        merged[key] = '__SAVED__'
        changed = true
      }
    }
    if (changed) onChange({ ...config, envVars: merged })
  }, [requiredVars, savedLookup, config, onChange])
  ...
  const handleNext = () => {
    ...
    if (trulyMissing.length > 0) {
      setValidationError(`Missing required variables: ${trulyMissing.join(', ')}`)
      return
    }
