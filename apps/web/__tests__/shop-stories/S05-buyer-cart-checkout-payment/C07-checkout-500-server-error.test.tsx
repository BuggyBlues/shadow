...
describe('S05/C07 checkout 500 server error', () => {
  ...
  it('服务异常时应报错', async () => {
    fetchApiMock.mockImplementation((path: string, options?: RequestInit) => {
      ...
      if (String(path).includes('/shop/orders') && options?.method === 'POST')
        return Promise.reject(new Error('shop.orderError(500)'))
      return Promise.resolve({})
    })
    ...
    await waitFor(() => expect(showToastMock).toHaveBeenCalledWith('shop.orderError(500)', 'error'))
  })
})
