  ...
  it('结算 payload 中 skuId 为 null 时应省略字段', async () => {
    fetchApiMock.mockImplementation((path: string, options?: RequestInit) => {
      if (String(path).includes('/shop/cart') && (!options?.method || options.method === 'GET')) {
        return Promise.resolve([{ id: 'c1', userId: 'u1', shopId: 's1', productId: 'p1', skuId: null, quantity: 1, product: { id: 'p1', name: '商品A', status: 'active', basePrice: 10 }, sku: null, imageUrl: null, unitPrice: 10 }])
      }
      if (String(path).includes('/shop/orders') && options?.method === 'POST') return Promise.resolve({ id: 'o1' })
    ...
    await waitFor(() => {
      const call = fetchApiMock.mock.calls.find((c) => String(c[0]).includes('/shop/orders') && c[1]?.method === 'POST')
