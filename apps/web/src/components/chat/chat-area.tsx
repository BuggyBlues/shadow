...
import { ArrowLeft, Check, ClipboardCopy, Copy, Hash, Loader2, LogIn, LogOut, UserPlus, Users, X } from 'lucide-react'
...
function InvitePanel({
  ...
  const { data: server } = useQuery({
    ...
    queryFn: () => fetchApi<{ id: string; inviteCode: string; slug: string }>(`/api/servers/${serverId}`),
    enabled: !!serverId,
  })
      ...
      <div className="bg-bg-secondary rounded-xl p-6 w-[520px] border border-border-subtle max-h-[80vh] overflow-hidden flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto space-y-1 pr-1">
          {activeTab === 'members' ? (
            ...
            <div className="text-center py-8 text-text-muted text-sm">
              暂无可用 Buddy，<a href="/app/buddy" className="text-primary hover:underline">去创建</a>
            </div>
          ) : (
