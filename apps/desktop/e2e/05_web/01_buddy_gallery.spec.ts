...
type Session = {
  ...
  buddies?: { names: string[]; ids: string[] }
}
...
test.describe
  .serial('buddy & buddy screenshot scenarios', () => {
    test('captures buddy management and buddy marketplace views', async ({ browser }) => {
      ...
      // Buddy management page — shows created buddies with status badges
      await ownerPage.goto('buddies')
      await ownerPage.waitForURL(/\/app\/buddies/)
      await ownerPage.waitForTimeout(1000)
      await screenshot(ownerPage, '13-buddy-buddy-management.png')

      // If buddies were seeded, click the first buddy to show details
      if (session.buddies?.names?.length) {
        // Wait for the buddy list to render in the sidebar
        const buddyCard = ownerPage.getByText(session.buddies.names[0]).first()
        try {
          await buddyCard.waitFor({ state: 'visible', timeout: 5000 })
          await buddyCard.click()
          await ownerPage.waitForTimeout(500)
          await screenshot(ownerPage, '14-buddy-detail-view.png')
        } catch {
          // Buddy card may not be visible — skip detail screenshot
        }
      }
